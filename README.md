...
Your Answer:
```Clearfix is a term to describe a common css pseudo element assigned to a parent element to force it to "clear" or span/encompass its children.
```
...
Your Answer:
```Scope defines which variables can be accessed at any given location in the code.
```Onlys function create new scope. Variables within a function defined with var are available in the global scope. 
...
Your Answer:
```js
$("#greeting").on("click",function(){$("<p>hello</p>").appendTo("#greeting")})
```
...
Your Answer:
```
https://github.com/Mahartney/Project-1
```
